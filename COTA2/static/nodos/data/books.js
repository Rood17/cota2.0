...
function draw() {
  // Agregado el 08/01/19 - categorias dentro de todas las funciones
  var datarel = document.getElementById('json_rel').value;
  var categorias = JSON.parse(datarel);
  ...
  var options = {
    ...
    color: {
        border: 'rgb(107, 159, 155)',
        background: 'rgb(67, 86, 100)'
      },
      font:{color:'#fff',size:0},
    },
    edges: {
        color: '#fff',
        smooth: false
  ...
  network.on("click", function (params) {
    ...
    var lId = objz.id - 1000000;
    document.getElementById('mlibro').innerHTML
    = "<a href='info_libro/" + lId + "' >[ Ver Más ]</a>";

    if (objz.libro !== undefined  ) {
        document.getElementById('autor').innerHTML
        = objz.libro.autor  ;
        
        Object.keys(objz.libro.cat_estudio).forEach(function(key) {
          if(ctr !== undefined){
            ctr =  ctr + '<a href="categoria_estudio/' + objz.libro.cat_id[key] + '/" >'
             + objz.libro.cat_estudio[key] + '</a> ';
          }else{
            ctr = '<a href="categoria_estudio/'
            + objz.libro.cat_id[key] + '">' + objz.libro.cat_estudio[key] + '</a> ';
          }
        });
...
function test(titulo){
  ...
  var ref     = new Array();
  // Se cambia data por dataa 27/12/18

  var dataa = document.getElementById('jsonh').value;
...
function buscar(){
      ...
      if (nodes.length > 1){
        ...
        if(titulo.length == 0){
          ...
          window.location = 'http://127.0.0.1:8000/nodos/sin_resultados/';
        }
      }
}
...
function bInicio(){
  ...
  if (titulo == 'stop') {
      console.log('se ha iniciado búsqueda por rel')
  } else {
      ...
      if (titulo.length == 0 ){
        window.location = 'http://127.0.0.1:8000/nodos/constelaciones/';
          ...
          if (nodes.length >= 1 ){
            ...
            // se necesita modificar en el servidor
            window.location = 'http://127.0.0.1:8000/nodos/sin_resultados/';
          } 
      }
  }
